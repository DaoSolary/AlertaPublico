...
export default function AlertasModal({
  ...
  const handleStatusChange = async (alertaId: number, newStatus: string, alerta?: any) => {
    try {
      ...
      const response = await api.put(`/alertas/${alertaId}`, payload)
                  ...
                  {status === 'PENDENTE' && (
                    <div className="flex flex-col gap-2 ml-4">
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          handleStatusChange(alerta.id, 'EM_ANDAMENTO', alerta)
                        }}
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          handleStatusChange(alerta.id, 'RESOLVIDO', alerta)
                        }}
